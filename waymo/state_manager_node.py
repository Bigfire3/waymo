...
class StateMachine(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "fallback_drivingspeed", 0.1
        )  # Fallback-Geschwindigkeit in m/s
        ...
        self.recommended_speed = self.get_parameter(
            "fallback_drivingspeed"
        ).value  # Initialwert
        ...
        self.traffic_light_subscription = self.create_subscription(
            Bool, "/traffic_light", self.traffic_light_callback, qos_reliable
        )
        self.keyboard_cmd_subscription = self.create_subscription(
            String, KEYBOARD_COMMAND_TOPIC, self.keyboard_command_callback, qos_reliable
        )
        self.sign_subscription = self.create_subscription(
            String, "/sign", self.sign_detection_callback, qos_reliable
        )
        self.parking_finished_subscription = self.create_subscription(
            Bool, "/parking/finished", self.parking_finished_callback, qos_reliable
        )
        self.intersection_finished_sub = self.create_subscription(
            Bool,
            "/intersection/finished",
            self.intersection_finished_callback,
            qos_reliable,
        )  # Name war vorher doppelt

        self.recommended_speed_subscription = self.create_subscription(
            Float64,
            "/robot/recommended_speed",
            self.recommended_speed_callback,
            qos_reliable,
        )

        self.state_publisher_ = self.create_publisher(
            String, "robot/state", qos_reliable
        )
        self.twist_publisher_ = self.create_publisher(Twist, "cmd_vel", qos_reliable)

        control_loop_period = 0.02
        self.control_timer = self.create_timer(
            control_loop_period, self.control_loop_callback
        )
    ...
    def toggle_manual_pause(self, toggle_command):
        if not self.manual_pause_active:
            ...
            pause_state_msg.data = STATE_MANUAL_PAUSE
            try:
            ...
            if toggle_command == "toggle_pause":
                ...
                self.obstacle_just_passed = (
                    False  # Reset, falls wir gerade in der Umfahrung waren
                )
                self.parking_sign_visually_detected = (
                    False  # Reset, falls wir gerade parken wollten
                )
                self.parking_maneuver_finished = (
                    False  # Reset, falls wir gerade parken wollten
                )
                self.straight_sign_visually_detected = False
                self.right_sign_visually_detected = False
                ...
                self.obstacle_just_passed = False
                self.parking_sign_visually_detected = (
                    True  # Setze Flag, um in PARKING zu wechseln
                )
                self.parking_maneuver_finished = (
                    False  # Reset, da wir jetzt parken wollen
                )
                self.straight_sign_visually_detected = False
                self.right_sign_visually_detected = False
                self.left_sign_visually_detected = False
                ...
                self.manual_pause_active = False
                self.traffic_light_is_red = (
                    True  # Setze Ampelstatus zurück, damit sie erneut geprüft wird
                )
                self.initial_traffic_light_check_done = (
                    False  # Ampelphase zurücksetzen, damit sie erneut geprüft wird
                )
                ...
                self.manual_pause_active = False
                self.initial_traffic_light_check_done = (
                    True  # Damit die Ampelphase nicht erneut geprüft wird
                )
    ...
    def obstacle_passed_callback(self, msg: Bool):
        if self.manual_pause_active:
            return
        if msg.data and (
            self.state == STATE_PASSING_OBSTACLE or self.state == STATE_FOLLOW_LANE
        ):
            self.obstacle_just_passed = True  # Wird in control_loop verarbeitet

    def traffic_light_callback(self, msg: Bool):
        if self.manual_pause_active or self.initial_traffic_light_check_done:
            return
        ...
        if self.traffic_light_is_red and not is_red:
            ...
            # Subscription jetzt sicher entfernen
    ...
    def control_loop_callback(self):
        ...
        if not self.initial_traffic_light_check_done:
            ...
            if (
                current_internal_state == STATE_PASSING_OBSTACLE
                and self.obstacle_just_passed
            ):
                next_state = STATE_FOLLOW_LANE
                self.obstacle_just_passed = False
                self.obstacle_is_blocking = False
            ...
            #    (Nicht während PASSING_OBSTACLE oder wenn an Ampel gestoppt)
            elif self.parking_sign_visually_detected and current_internal_state not in [
                STATE_PASSING_OBSTACLE,
                STATE_PARKING,
                STATE_STOPPED_AT_TRAFFIC_LIGHT,
                STATE_INTERSECTION_DRIVING_STRAIGHT,
                STATE_INTERSECTION_TURNING_LEFT,
                STATE_INTERSECTION_TURNING_RIGHT,
            ]:  # Intersection States auch ausschließen
                next_state = STATE_PARKING
                self.parking_sign_visually_detected = False
                self.parking_maneuver_finished = False

            elif (
                current_internal_state == STATE_PARKING
                and self.parking_maneuver_finished
            ):
                next_state = STATE_FOLLOW_LANE
                self.parking_sign_visually_detected = False
                self.parking_maneuver_finished = False
            ...
            elif self.obstacle_is_blocking and current_internal_state not in [
                STATE_PASSING_OBSTACLE,
                STATE_PARKING,
                ...
                current_internal_state == STATE_STOPPED_AT_OBSTACLE
                and self.obstacle_is_blocking
            ):
            ...
            # l) Manöver abgeschlossen
                ...
                current_internal_state == STATE_INTERSECTION_DRIVING_STRAIGHT
            ...
            # i) Geradeaus-Schild
                ...
                self.straight_sign_visually_detected
                and current_internal_state
                not in [
                    STATE_PASSING_OBSTACLE,
                    STATE_PARKING,
                    STATE_STOPPED_AT_TRAFFIC_LIGHT,
            ...
            # j) Rechts-Schild
            elif self.right_sign_visually_detected and current_internal_state not in [
                STATE_PASSING_OBSTACLE,
                STATE_PARKING,
                STATE_STOPPED_AT_TRAFFIC_LIGHT,
                STATE_INTERSECTION_DRIVING_STRAIGHT,
            ...
            # k) Links-Schild
            elif self.left_sign_visually_detected and current_internal_state not in [
                STATE_PASSING_OBSTACLE,
                STATE_PARKING,
                STATE_STOPPED_AT_TRAFFIC_LIGHT,
                STATE_INTERSECTION_DRIVING_STRAIGHT,
            ...
            # n) Fallback auf FOLLOW_LANE wenn in FOLLOW_LANE und nichts anderes zutrifft
                ...
                self.obstacle_is_blocking
                or self.parking_sign_visually_detected
                or self.straight_sign_visually_detected
                or self.right_sign_visually_detected
                or self.left_sign_visually_detected
            ):  # intersection_maneuver_finished ist hier nicht relevant
    ...
    def publish_current_state(self):
        ...
        state_to_publish = (
            STATE_MANUAL_PAUSE if self.manual_pause_active else self.state
        )
        state_msg = String()
        state_msg.data = state_to_publish
        try:
            if rclpy.ok() and self.context.ok():
    ...
    def destroy_node(self):
        try:
            ...
            self.get_logger().warn(
                f"Fehler beim Senden des Stoppbefehls in destroy_node: {e}"
            )
        super().destroy_node()
    ...
    try:
        ...
        if node:
            node.get_logger().error(
                f"FATAL ERROR [SM] in main: {e}\n{traceback.format_exc()}"
            )
        else:
            print(
                f"FATAL ERROR [SM] vor Node-Init: {e}\n{traceback.format_exc()}",
                file=sys.stderr,
            )
    finally:
        if node is not None:
