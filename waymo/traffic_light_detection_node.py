...
class TrafficLightDetector(Node):
    def __init__(self):
        ...
        def int_area_desc(
            desc, min_val=0, max_val=5000, step=1
        ):  # Max Area wie im Original
            return ParameterDescriptor(
                type=ParameterType.PARAMETER_INTEGER,
                description=desc,
                integer_range=[
        ...
        self.declare_parameter(
            "hsv_lower_h1", 0, int_desc("Lower Hue 1 (0-180)", max_val=180)
        )
        self.declare_parameter(
            "hsv_upper_h1", 10, int_desc("Upper Hue 1 (0-180)", max_val=180)
        )
        ...
        self.declare_parameter(
            "hsv_lower_s", 50, int_desc("Lower Saturation (0-255)")
        )  # Startwert erhöht
        self.declare_parameter(
            "hsv_lower_v", 10, int_desc("Lower Value (0-255)")
        )  # Startwert erhöht
        self.declare_parameter("hsv_upper_s", 255, int_desc("Upper Saturation (0-255)"))
        self.declare_parameter(
            "hsv_upper_v", 255, int_desc("Upper Value (0-255)")
        )  # Oberen Wert evtl. anpassen
        # Blob Area
        self.declare_parameter(
        ...
        self.declare_parameter(
            ...
            0.6,
            float_desc(
                "ROI Crop Factor Height (0.1-1.0)", min_val=0.1, max_val=1.0, step=0.01
            ),
        )

        # Debug Publisher Flags
        self.declare_parameter(
            "publish_mask", True, bool_desc("Publish filtered color mask image")
        )
        self.declare_parameter(
            "publish_overlay", True, bool_desc("Publish color detection overlay image")
        )
        ...
        self.pub_mask = self.create_publisher(
            CompressedImage, "/debug/cam/traffic_mask", qos_best_effort
        )
        ...
        try:
            ...
            if publish_overlay_flag:
                if frame_cropped.size > 0:
                    ...
                    overlay[filtered_mask > 0] = (
                        0,
                        255,
                        0,
                    )  # Grün für erkannte Bereiche
            ...
            self.get_logger().error(
                f"ERROR in image_callback: {e}", throttle_duration_sec=10
            )
    ...
    def detect_target_color(self, frame):
        ...
        try:
            ...
            mask_closed = cv2.morphologyEx(
                mask_opened, cv2.MORPH_CLOSE, kernel, iterations=1
            )
            ...
            self.get_logger().error(
                f"OpenCV error in detect_target_color: {cv_err}",
                throttle_duration_sec=10,
            )
        except Exception as e:
            self.get_logger().error(
                f"Error in detect_target_color: {e}", throttle_duration_sec=10
            )
            self.get_logger().error(traceback.format_exc())
...
def main(args=None):
    ...
    try:
        ...
        if rclpy.ok():
            rclpy.shutdown()
        try:
            cv2.destroyAllWindows()  # Sicherstellen, dass alle Fenster geschlossen werden
        except Exception:
            pass
