...
class LaneDetectionNode(Node):
    def __init__(self):
        ...
        def float_desc(desc, min_val=0.0, max_val=1.0, step=0.001):
            return ParameterDescriptor(
        ...
        self.declare_parameter(
            "center_factor", 0.015, float_desc("Center_Calc: factor")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "y_bottom_height",
            80,
            int_desc("Y-Bottom-Height", min_val=0, max_val=144, step=1),
        )  # Schrittweite 1
        self.declare_parameter("smoothing_on_off", False, bool_desc("Smoothing on/off"))
        self.declare_parameter(
            "weight_previous_fit",
            0.75,
            float_desc("Weight previous fit", min_val=0.0, max_val=1.0, step=0.01),
        )
        self.declare_parameter(
            "weight_current_fit",
            0.25,
            float_desc("Weight current fit", min_val=0.0, max_val=1.0, step=0.01),
        )
        ...
        self.declare_parameter(
            ...
            1.5,
            float_desc(
                "Minimum thickness ratio", min_val=0.01, max_val=200.0, step=0.001
            ),
        )
        self.declare_parameter(
            "max_thickness",
            5.0,
            float_desc(
                "Maximum thickness ratio", min_val=0.01, max_val=200.0, step=0.001
            ),
        )
        # ROI Parameter
        self.declare_parameter(
            "roi_top_left_w", 0.0, float_desc("ROI TL Breite")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_top_left_h", 0.7, float_desc("ROI TL Höhe")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_top_right_w", 1.0, float_desc("ROI TR Breite")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_top_right_h", 0.7, float_desc("ROI TR Höhe")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_bottom_left_w", 0.0, float_desc("ROI BL Breite")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_bottom_left_h", 1.0, float_desc("ROI BL Höhe")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_bottom_right_w", 1.0, float_desc("ROI BR Breite")
        )  # Schrittweite 0.001
        self.declare_parameter(
            "roi_bottom_right_h", 1.0, float_desc("ROI BR Höhe")
        )  # Schrittweite 0.001
        # desired_roi_padding_factor hat step=0.01, Standardwert 0.25 ist gültig
        self.declare_parameter(
            "desired_roi_padding_factor",
            0.25,
            float_desc("Ziel ROI Padding", max_val=0.4, step=0.01),
        )
        ...
        self.declare_parameter(
            "aspect_ratio",
            2.0,
            float_desc("aspect_ratio", min_val=0.1, max_val=10.0, step=0.1),
        )
        self.declare_parameter(
            "compactness",
            0.015,
            float_desc("compactness", min_val=0.001, max_val=0.2, step=0.001),
        )

        self.declare_parameter(
            "sliding_window_margin_factor",
            0.1,
            float_desc("sliding_window_margin_factor"),
        )
        self.declare_parameter(
            "sliding_window_minpix", 50, float_desc("sliding_window_minpix")
        )
        ...
        self.declare_parameter(
            "min_contour_area", 50, int_desc("min_contour_area", max_val=10000)
        )
        self.declare_parameter(
            "max_contour_area", 1500, int_desc("max_contour_area", max_val=100000)
        )

        # --- HINZUGEFÜGT: Parameter zum Steuern der Debug-Publisher ---
        self.declare_parameter(
            "publish_roi_image", True, bool_desc("Publish ROI visualization image")
        )
        self.declare_parameter(
            "publish_lane_annotated",
            True,
            bool_desc("Publish final annotated lane image"),
        )
        self.declare_parameter(
            "publish_closed_edges", True, bool_desc("Publish closed edges image")
        )
        self.declare_parameter(
            "publish_filled_areas", True, bool_desc("Publish filled areas image")
        )
        self.declare_parameter(
            "publish_thickness_filtered",
            True,
            bool_desc("Publish thickness-filtered warped image"),
        )
        self.declare_parameter(
            "publish_sliding_window",
            True,
            bool_desc("Publish sliding window search visualization"),
        )
        # --- Ende HINZUGEFÜGT ---
        ...
        self.center_offset_publisher_ = self.create_publisher(
            Float64, "/lane/center_offset", qos_control_data
        )
        self.curvature_publisher_ = self.create_publisher(
            Float64, "/lane/curvature", qos_control_data
        )

        # --- Debug Publisher (unverändert) ---
        self.pub_roi = self.create_publisher(
            CompressedImage, "/debug/cam/roi", qos_debug_images
        )
        self.pub_lane_annotated = self.create_publisher(
            CompressedImage, "/debug/cam/lane_annotated", qos_debug_images
        )
        self.pub_closed_edges = self.create_publisher(
            CompressedImage, "/debug/cam/closed_edges", qos_debug_images
        )
        self.pub_filled_areas = self.create_publisher(
            CompressedImage, "/debug/cam/filled_areas", qos_debug_images
        )
        self.pub_thickness_filtered = self.create_publisher(
            CompressedImage, "/debug/cam/thickness_filtered", qos_debug_images
        )
        self.pub_sliding_window = self.create_publisher(
            CompressedImage, "/debug/cam/sliding_window", qos_debug_images
        )
    ...
    def _publish_image(self, publisher, image, timestamp):
        ...
        try:
            ...
            self.get_logger().error(
                f"CvBridge Fehler beim Publishen auf '{publisher.topic}': {e}",
                throttle_duration_sec=5,
            )
        except Exception as e:
            self.get_logger().error(
                f"Allgemeiner Fehler beim Publishen auf '{publisher.topic}': {e}",
                throttle_duration_sec=5,
            )
        ...
        try:
            ...
            if self.frame is None or self.gray_scale_frame is None:
                self.get_logger().warn(
                    "Dekodiertes Bild ist leer oder ungültig. Überspringe Bildverarbeitung.",
            ...
            for key in [
                "publish_lane_annotated",
                "publish_filled_areas",
                "publish_warped_frame",
                "publish_thickness_filtered",
                "publish_sliding_window",
            ]:
            ...
            warped_frame = self.lane_obj.perspective_transform(
                frame=self.gray_scale_frame, plot=False
            )
            ...
            if current_params.get("publish_closed_edges"):
                self._publish_image(
                    self.pub_closed_edges, closed_edges_frame, timestamp
                )
            ...
            if current_params.get("publish_filled_areas"):
                self._publish_image(
                    self.pub_filled_areas, filled_areas_frame, timestamp
                )

            # 3. Nach Dicke filtern (Verwendet min/max_thickness)
            self.lane_obj.filter_lane_markings_by_thickness(
                filled_areas_frame, plot=False, **current_params
            )
            if current_params.get("publish_thickness_filtered"):
                self._publish_image(
                    self.pub_thickness_filtered,
                    self.lane_obj.filtered_warped_frame,
                    timestamp,
                )
            ...
            left_fit, right_fit, sliding_window_img = (
                self.lane_obj.get_lane_line_indices_sliding_windows(plot=True)
            )  # plot=False, da Bild zurückkommt
            if (
                current_params.get("publish_sliding_window")
                and sliding_window_img is not None
            ):
                self._publish_image(
                    self.pub_sliding_window, sliding_window_img, timestamp
                )

            # 6. Overlay, Krümmung und Offset berechnen
            final_annotated_frame = self.frame
            if (
                self.lane_obj.left_fit is not None
                and self.lane_obj.right_fit is not None
            ):
                ...
                self.lane_obj.calculate_curvature(
                ...
                final_annotated_frame, curvature = (
                    self.lane_obj.display_curvature_offset(
                        frame=overlay_frame, plot=False
                    )
                )
            ...
            if current_params.get("publish_lane_annotated"):
                self._publish_image(
                    self.pub_lane_annotated, final_annotated_frame, timestamp
                )
            ...
            self.get_logger().error(
                f"CvBridge Error in image_callback: {e}", throttle_duration_sec=10
            )
        except Exception as e:
            self.get_logger().error(
                f"Error processing image: {e}", throttle_duration_sec=10
            )
...
def main(args=None):
    ...
    try:
        ...
        if node:
            ...
            print("KeyboardInterrupt received before node init.", file=sys.stderr)
    except Exception as e:
        # Logge Fehler, bevor Node zerstört wird
        if node:
            node.get_logger().error(
                f"FATALER FEHLER in LaneDetectionNode: {e}\n{traceback.format_exc()}"
            )
        else:
            print(
                f"FATALER FEHLER vor Node-Init: {e}\n{traceback.format_exc()}",
                file=sys.stderr,
            )
    finally:
