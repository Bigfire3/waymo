...
class SignDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "publish_binary_sign",
            True,
            bool_desc("Publish binary frame with colored detection boxes as cv2 image"),
        )
        self.declare_parameter(
            "binary_threshold", 80, int_desc("Threshold for binary image")
        )
        ...
        self.pub_binary_sign_with_box = self.create_publisher(
            CompressedImage, "/debug/cam/binary_sign_boxed", qos_best_effort
        )
        ...
        if not self.templates:
            self.get_logger().error(
                "Keine Templates wurden geladen! Überprüfe den Pfad und die setup.py Konfiguration."
            )
        else:
            for name, template in self.templates.items():
                if template is None:
                    self.get_logger().error(
                        f"Template '{name}' konnte nicht korrekt initialisiert werden (ist None)."
                    )
    ...
    def _publish_image(self, publisher, image, timestamp):
        if image is None:
            self.get_logger().warn(
                f"Versuch, ein None-Bild auf Topic '{publisher.topic}' zu publishen.",
        ...
        try:
            ...
            if processed_image.dtype != np.uint8:
                if np.max(processed_image) <= 1.0 and (
                    processed_image.dtype == np.float32
                    or processed_image.dtype == np.float64
                ):
                    processed_image = (processed_image * 255).astype(np.uint8)
                else:
                    processed_image = processed_image.astype(np.uint8)
            ret, buffer = cv2.imencode(
                ".jpg", processed_image, [int(cv2.IMWRITE_JPEG_QUALITY), 90]
            )
            if not ret:
                self.get_logger().warn(
                    f"Fehler beim JPEG-Encoding für Topic '{publisher.topic}'."
                )
                return
    ...
    def load_templates(self, folder_path):
        ...
        if not os.path.isdir(folder_path):
            self.get_logger().error(
                f"Template-Ordner nicht gefunden oder ist kein Verzeichnis: {folder_path}"
            )
            return templates
            ...
            if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                try:
                    ...
                    if template is not None:
                        ...
                        self.get_logger().error(
                            f"Fehler beim Laden von Template '{filename}' (imread gab None zurück) von Pfad: {image_path}"
                        )
                except Exception as e:
                    self.get_logger().error(
                        f"Ausnahme beim Laden von Template '{filename}' von Pfad '{image_path}': {e}"
                    )
        if not templates:
            self.get_logger().warn(
                f"Keine Templates im Ordner '{folder_path}' gefunden oder geladen."
            )
        return templates

    def convert_to_binary(
        self, image, binary_threshold_value
    ):  # Nimmt jetzt den Threshold-Wert entgegen
        if image is None:
            self.get_logger().error(
                "Versuch, ein None-Bild zu konvertieren (convert_to_binary)."
            )
            return None
        ...
        _, binary_image = cv2.threshold(
            gray_image, binary_threshold_value, 255, cv2.THRESH_BINARY
        )
    ...
    def listener_callback(self, msg):
        try:
            ...
            self.get_logger().error(
                f"Fehler beim Dekodieren des komprimierten Bildes: {e}"
            )
            return
        ...
        image_roi_color = image_color_input[
        ...
        _, image_bin_roi_for_detection = cv2.threshold(
            image_gray_roi_for_detection, binary_threshold, 255, cv2.THRESH_BINARY
        )
        ...
        if self.get_parameter("publish_binary_sign").value:
            ...
            _, image_bin_full_for_debug = cv2.threshold(
                image_gray_full, binary_threshold, 255, cv2.THRESH_BINARY
            )
            debug_image_base = cv2.cvtColor(
                image_bin_full_for_debug, cv2.COLOR_GRAY2BGR
            )
            # Zeichne die ROI-Grenze in das Debug-Bild
            cv2.rectangle(
                debug_image_base,
                (roi_x_start, roi_y_start),
                (roi_x_start + roi_width - 1, roi_y_start + roi_height - 1),
                (147, 20, 255),
                2,
            )  # Lila ROI-Box
        ...
        current_templates_bin = {
            name: self.convert_to_binary(template, binary_threshold)
            for name, template in self.templates.items()
            if template is not None
        }
        ...
        for template_name, template_bin in current_templates_bin.items():
            ...
            if (
                image_bin_roi_for_detection.shape[0] < h
                or image_bin_roi_for_detection.shape[1] < w
            ):
            ...
            result = cv2.matchTemplate(
                image_bin_roi_for_detection, template_bin, cv2.TM_CCOEFF_NORMED
            )
            min_val, max_val, min_loc, max_loc_roi = cv2.minMaxLoc(
                result
            )  # max_loc_roi ist relativ zur ROI

            if max_val > detection_confidence:
                # Konvertiere lokale ROI-Koordinaten zu globalen Bildkoordinaten
                top_left_global = (
                    max_loc_roi[0] + roi_x_start,
                    max_loc_roi[1] + roi_y_start,
                )
                ...
                if debug_image_base is not None:
                    ...
                    cv2.putText(
                        debug_image_base,
                        template_name,
                        (top_left_global[0], top_left_global[1] - 10),
                ...
                if not detected_signs_on_current_frame:
                    ...
                    if sign_msg_data:
                        ...
                        detected_signs_on_current_frame = (
                            True  # Nur das erste erkannte Schild pro Frame senden
                        )

        if (
            self.get_parameter("publish_binary_sign").value
            and debug_image_base is not None
        ):
            self._publish_image(
                self.pub_binary_sign_with_box, debug_image_base, timestamp
            )
