...
class Lane:
    ...
    def update_frame(self, orig_frame, **params):
        ...
        self.roi_points = np.float32([
            (w * params.get('roi_top_left_w', 0.15), h * params.get('roi_top_left_h', 0.70)),   # top-left
            (w * params.get('roi_bottom_left_w', 0.0), h * params.get('roi_bottom_left_h', 1.0)),     # bottom-left
            (w * params.get('roi_bottom_right_w', 1.0), h * params.get('roi_bottom_right_h', 0.85)),     # bottom-right
            (w * params.get('roi_top_right_w', 0.85), h * params.get('roi_top_right_h', 0.55))     # top-right
        ])
    ...
    def calculate_curvature(self, print_to_terminal=False):
        ...
        left_fit_cr = np.polyfit(self.lefty * self.YM_PER_PIX, self.leftx * 0.00475, 2)
        right_fit_cr = np.polyfit(self.righty * self.YM_PER_PIX, self.rightx * 0.00475, 2)
        left_curvem = ((1 + (2 * left_fit_cr[0] * y_eval * self.YM_PER_PIX + left_fit_cr[1])**2)**1.5) / abs(2 * left_fit_cr[0])
        right_curvem = ((1 + (2 * right_fit_cr[0] * y_eval * self.YM_PER_PIX + right_fit_cr[1])**2)**1.5) / abs(2 * right_fit_cr[0])
        
        if print_to_terminal:
            print(f"Left Curvature: {left_curvem} m, Right Curvature: {right_curvem} m")
    ...
    def get_lane_line_indices_sliding_windows(self, plot=False):
        ...
        peaks = self.select_lane_peaks(self.histogram, target_distance=250, min_peak_value=5000)
        ...
        for i in range(len(peaks)):
            for j in range(i+1, len(peaks)):
                ...
                if diff < best_diff and diff < 20:
                    best_diff = diff
                    best_pair = (min(peaks[i], peaks[j]), max(peaks[i], peaks[j]))
    ...
    def overlay_lane_lines(self, plot=False):
        ...
        if self.previous_bottom_left_x is not None and self.previous_bottom_right_x is not None:
            lane_center = int((self.previous_bottom_left_x + self.previous_bottom_right_x) / 2)
        else:
            current_left_x = self.left_fit[0]*y_bottom**2 + self.left_fit[1]*y_bottom + self.left_fit[2]
            current_right_x = self.right_fit[0]*y_bottom**2 + self.right_fit[1]*y_bottom + self.right_fit[2]
            lane_center = int((current_left_x + current_right_x) / 2)
        if self.current_center is not None:
            cv2.circle(result, (int(self.current_center), y_bottom-20), 5, (0, 0, 255), -1)
        if plot:
            fig, (ax1, ax2) = plt.subplots(2, 1)
    ...
    def plot_roi(self, frame=None, plot=False):
        ...
        this_image = cv2.polylines(frame, [np.int32(self.roi_points)], True, (147,20,255), 2)
