...
class Lane:
    ...
    def update_frame(self, orig_frame, **params):
        ...
        self.roi_points = np.float32(
            [
                (
                    w * params.get("roi_top_left_w", 0.1),
                    h * params.get("roi_top_left_h", 0.65),
                ),  # top-left
                (
                    w * params.get("roi_bottom_left_w", 0.0),
                    h * params.get("roi_bottom_left_h", 1.0),
                ),  # bottom-left
                (
                    w * params.get("roi_bottom_right_w", 1.0),
                    h * params.get("roi_bottom_right_h", 1.0),
                ),  # bottom-right
                (
                    w * params.get("roi_top_right_w", 0.9),
                    h * params.get("roi_top_right_h", 0.65),
                ),  # top-right
            ]
        )
    ...
    def calculate_car_position(self, **params):
        ...
        try:
            current_right_x = (
                self.right_fit[0] * self.y_bottom**2
                + self.right_fit[1] * self.y_bottom
                + self.right_fit[2]
            )
            # if (self.left_fit[0] * self.right_fit[0]) > 0:
            current_left_x = (
                self.left_fit[0] * self.y_bottom**2
                + self.left_fit[1] * self.y_bottom
                + self.left_fit[2]
            )
            # else:
        ...
        center_factor = params.get(
            "center_factor", 0.03
        )  # Standardwert wie im Original-Node
    ...
    def calculate_curvature(self, print_to_terminal=False, **params):
        if (
            self.left_fit is None
            or self.right_fit is None
            or self.lefty is None
            or self.righty is None
            or self.ploty is None
            or len(self.lefty) == 0
            or len(self.righty) == 0
        ):
            if print_to_terminal:
                print("Keine Fahrspurlinie für Krümmungsberechnung erkannt")
            self.left_curvem = None
            self.right_curvem = None
            return None, None
        ...
        try:
            ...
            print("Fehler beim Polyfit für Krümmung.")
            self.left_curvem = None
            self.right_curvem = None
            return None, None
        ...
        if abs(left_a) < 1e-6:
            self.left_curvem = float("inf")
        else:
            self.left_curvem = (
                (1 + (2 * left_a * y_eval + left_fit_cr[1]) ** 2) ** 1.5
            ) / np.absolute(2 * left_a)
        if abs(right_a) < 1e-6:
            self.right_curvem = float("inf")
        else:
            self.right_curvem = (
                (1 + (2 * right_a * y_eval + right_fit_cr[1]) ** 2) ** 1.5
            ) / np.absolute(2 * right_a)

        if print_to_terminal:
            left_str = (
                f"{self.left_curvem:.1f} m"
                if self.left_curvem != float("inf")
                else "straight"
            )
            right_str = (
                f"{self.right_curvem:.1f} m"
                if self.right_curvem != float("inf")
                else "straight"
            )
    ...
    def calculate_histogram(self, frame=None, plot=False):
        ...
        if plot:
            try:
                ...
                ax1.set_title("Warped Binary Frame for Histogram")
                ax2.plot(self.histogram)
                ax2.set_title("Histogram Peaks")
                plt.show()
            except Exception as e:
                print(f"Fehler beim Plotten des Histogramms: {e}")
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        valid_curves = [
            c
            for c in [self.left_curvem, self.right_curvem]
            if c is not None and c != float("inf")
        ]
        if valid_curves:
            avg_curve = np.mean(valid_curves)
            curve_radius_text = (
                "Curve Radius: straight"
                if avg_curve > 5000
                else f"Curve Radius: {avg_curve:.1f} m"
            )
        elif self.left_curvem == float("inf") or self.right_curvem == float("inf"):
            curve_radius_text = "Curve Radius: straight"
        cv2.putText(
            image_copy,
            curve_radius_text,
            pos_curve,
        ...
        if self.center_offset is not None:
            offset_text = f"Center Offset: {self.center_offset:.3f}"  # Einheit unklar
        cv2.putText(
            image_copy,
            offset_text,
            pos_offset,
        ...
        if plot:
            try:
                ...
                print(f"Fehler beim Anzeigen von Curvature/Offset: {e}")
    ...
    def get_lane_line_indices_sliding_windows(self, plot=False, **params):
        ...
        nonzero = frame_sliding_window.nonzero()
        nonzeroy = np.array(nonzero[0])
        nonzerox = np.array(nonzero[1])

        target_distance = (
            params.get("histogram_peak_target_distance_factor", 0.4) * self.width
        )
        min_peak_value = params.get(
            "histogram_min_peak_value", 5000
        )  # Wert aus Original
        distance_tolerance = (
            params.get("histogram_distance_tolerance_factor", 0.1) * self.width
        )
        ...
        if peaks is None or peaks[0] is None or peaks[1] is None:
            if (
                self.previous_left_fit is not None
                and self.previous_right_fit is not None
            ):
                ...
                if self.ploty is None:
                    self.ploty = np.linspace(
                        0,
                        self.filtered_warped_frame.shape[0] - 1,
                        self.filtered_warped_frame.shape[0],
                    )
                try:
                    self.left_fitx = (
                        self.left_fit[0] * self.ploty**2
                        + self.left_fit[1] * self.ploty
                        + self.left_fit[2]
                    )
                    self.right_fitx = (
                        self.right_fit[0] * self.ploty**2
                        + self.right_fit[1] * self.ploty
                        + self.right_fit[2]
                    )
                except (TypeError, IndexError):
                    self.left_fit = None
        ...
        for window in range(self.no_of_windows):
            win_y_low = (
                self.filtered_warped_frame.shape[0] - (window + 1) * window_height
            )
        ...
        if new_right_fit is not None:
            if use_smoothing and self.previous_right_fit is not None:
                self.right_fit = (
                    weight_current_fit * new_right_fit
                    + weight_previous_fit * self.previous_right_fit
                )
            else:
        ...
        if self.left_fit is not None and self.right_fit is not None:
            if self.ploty is None or len(self.ploty) != frame_sliding_window.shape[0]:
                self.ploty = np.linspace(
        ...
        if plot and out_img is not None:
            try:
                ...
                if self.right_fitx is not None and self.ploty is not None:
                    pts_right = np.array(
                        [np.transpose(np.vstack([self.right_fitx, self.ploty]))]
                    )
                    cv2.polylines(
                        out_img,
                        np.int_([pts_right]),
                ...
                print(f"Fehler beim Erstellen des Sliding Window Debug-Bildes: {e}")
                self.sliding_window_debug_img = None
        else:
            self.sliding_window_debug_img = None
    ...
    @staticmethod
    def select_lane_peaks(
        histogram, target_distance, min_peak_value, distance_tolerance
    ):
        # --- Code wie zuvor ---
        ...
        for i in range(1, len(histogram) - 1):
            if (
                ...
                and histogram[i] >= min_peak_value
            ):
        ...
        for i in range(len(peaks_indices)):
            for j in range(i + 1, len(peaks_indices)):
                peak1_idx = peaks_indices[i]
                peak2_idx = peaks_indices[j]
                ...
                if diff <= distance_tolerance and diff < min_diff_to_target:
                    ...
                    best_pair = (min(peak1_idx, peak2_idx), max(peak1_idx, peak2_idx))
        if best_pair is None and len(peaks_indices) >= 2:  # Fallback
            sorted_peaks = sorted(
                peaks_indices, key=lambda idx: histogram[idx], reverse=True
            )
            strongest_pair = sorted(sorted_peaks[:2])
            if abs(strongest_pair[1] - strongest_pair[0]) > 0.1 * len(histogram):
                best_pair = (strongest_pair[0], strongest_pair[1])
    ...
    def overlay_lane_lines(self, plot=True):
        if (
            ...
            or self.right_fit is None
            or self.ploty is None
            or self.left_fitx is None
            or self.right_fitx is None
            or self.inv_transformation_matrix is None
        ):
        ...
        pts_left = (
            np.vstack([self.left_fitx + 160, self.ploty])
            .T[np.newaxis, ...]
            .astype(np.float32)
        )
        pts_right = np.flipud(np.vstack([self.right_fitx + 160, self.ploty]).T)[
            np.newaxis, ...
        ].astype(np.float32)
    ...
    def perspective_transform(self, frame=None, plot=False):
        try:
            ...
            print(f"Fehler bei getPerspectiveTransform: {e}")
            frame = None
            self.transformation_matrix = None
            self.inv_transformation_matrix = None
            return
        try:
            ...
            warped_frame = cv2.warpPerspective(
                frame, self.transformation_matrix, (w, h), flags=cv2.INTER_LINEAR
            )
        except cv2.error as e:
            print(f"Fehler bei warpPerspective: {e}")
            frame = None
            return
    ...
    def filter_lane_markings_by_thickness(self, frame, plot=False, **params):
        ...
        if plot:
            try:
                cv2.imshow("Filtered Lane Markings (Thickness)", filtered_mask)
                cv2.waitKey(1)
            except Exception as e:
                print(f"Fehler beim Anzeigen der gefilterten Markierungen: {e}")

    def plot_roi(
        self, frame=None
    ):  # Entferne plot=False Argument, gibt jetzt immer Bild zurück
        ...
        try:
            ...
            ]  # Muss eine Liste von Arrays sein
            # Zeichne auf die Kopie
            cv2.polylines(
                frame, roi_drawable, True, (147, 20, 255), 2
            )  # Lila Farbe, Dicke 2
