...
class PassingObstacleNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.state_subscriber = self.create_subscription(
            String, "/robot/state", self.robot_state_callback, qos_reliable
        )
        self.offset_subscription = self.create_subscription(
            Float64, "/lane/center_offset", self.lane_offset_callback, qos_sensor
        )
        ...
        self.passed_publisher = self.create_publisher(
            Bool, "/obstacle/passed", qos_reliable
        )
    ...
    def odom_callback(self, msg: Odometry):
        ...
        #     return # Keine Verarbeitung, wenn nicht aktiv
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.maneuver_state != ManeuverState.CHECKING_SIDE:
            return
        target_angle_min_rad = SIDE_SCAN_ANGLE_MIN
        target_angle_max_rad = SIDE_SCAN_ANGLE_MAX
        ...
        try:
            ...
            if angle_increment_rad <= 0.0:
                self.side_is_clear = False
                return  # Fehler ohne Log
            adj_target_min = max(target_angle_min_rad, scan_angle_min_rad)
            adj_target_max = min(target_angle_max_rad, scan_angle_max_rad)
            min_index = max(
                0, int((adj_target_min - scan_angle_min_rad) / angle_increment_rad)
            )
            max_index = min(
                num_ranges - 1,
                int((adj_target_max - scan_angle_min_rad) / angle_increment_rad),
            )
    ...
    def run_maneuver(self):
        ...
        if self.maneuver_state == ManeuverState.TURNING_LEFT_1:
            if self.turn_to_target(self.target_yaw, ANGULAR_SPEED):
                self.maneuver_state = ManeuverState.MOVING_SIDEWAYS_1
                self.start_time = current_time
        elif self.maneuver_state == ManeuverState.MOVING_SIDEWAYS_1:
            if self.move_forward_for_duration(
                LINEAR_SPEED, MOVE_DURATION_SIDEWAYS, current_time
            ):
                self.maneuver_state = ManeuverState.TURNING_RIGHT_1
                self.start_yaw = self.current_yaw
                self.target_yaw = self.normalize_angle(
                    self.start_yaw - TURN_ANGLE_90_DEG
                )
        elif self.maneuver_state == ManeuverState.TURNING_RIGHT_1:
            if self.turn_to_target(self.target_yaw, -ANGULAR_SPEED):
                self.maneuver_state = ManeuverState.WAIT_BEFORE_CHECKING
            ...
            angular_z = np.clip(
                angular_z, -MAX_ANGULAR_Z_PASSING, MAX_ANGULAR_Z_PASSING
            )
            if current_time - self.wait_start_time < DRIVE_DURATION_BEFORE_SIDE_CHECK:
                ...
                if self.side_is_clear:
                    self.stop_robot()
                    self.maneuver_state = ManeuverState.TURNING_RIGHT_2
                    self.start_yaw = self.current_yaw
                    self.target_yaw = self.normalize_angle(
                        self.start_yaw - TURN_ANGLE_90_DEG
                    )
            ...
            if self.turn_to_target(self.target_yaw, -ANGULAR_SPEED):
                self.maneuver_state = ManeuverState.MOVING_SIDEWAYS_2
                self.start_time = current_time
        elif self.maneuver_state == ManeuverState.MOVING_SIDEWAYS_2:
            if self.move_forward_for_duration(
                LINEAR_SPEED, MOVE_DURATION_SIDEWAYS, current_time
            ):
                self.maneuver_state = ManeuverState.TURNING_LEFT_2
                self.start_yaw = self.current_yaw
                self.target_yaw = self.normalize_angle(
                    self.start_yaw + TURN_ANGLE_90_DEG
                )
            ...
            if self.turn_to_target(self.target_yaw, ANGULAR_SPEED):
                self.maneuver_state = ManeuverState.COMPLETE
