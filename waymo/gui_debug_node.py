...
class GuiDebugNode(Node):
    def __init__(self):
        ...
        scale_factor_descriptor = ParameterDescriptor(
            ...
            floating_point_range=[
                FloatingPointRange(from_value=0.0001, to_value=2.0, step=0.0001)
            ],
        )
        ...
        for name, topic in self.debug_topics.items():
            callback = lambda msg, topic_name=name: self.debug_image_callback(
                msg, topic_name
            )
    ...
    def _create_placeholder_image(
        self, width, height, text="Warte..."
    ):  # Nimmt jetzt w/h als Argument
    ...
    def debug_image_callback(self, msg, topic_name):
        ...
        try:
            ...
            self.get_logger().warn(
                f"CvBridge Error für '{topic_name}': {e}", throttle_duration_sec=10
            )
            if topic_name in self.last_images:
                self.last_images[topic_name] = None
        except Exception as e:
            self.get_logger().warn(
                f"Fehler beim Verarbeiten von '{topic_name}': {e}",
                ...
                if not self.gui_initialized:
                    try:
                        ...
                        self.get_logger().error(
                            f"Fensterfehler: {e}. Canvas deaktiviert."
                        )
                        self.canvas_visible = False
            else:
    ...
    def update_canvas(self):
            ...
            if img is not None:
                try:
                    ...
                    if w == 0 or h == 0:  # Ungültiges Bild
                        tile_img = self._create_placeholder_image(
                            target_w, target_h, f"{name} (Invalid)"
                        )
                    else:
                        ...
                        if new_w <= 0 or new_h <= 0:  # Fehler bei Skalierung
                            tile_img = self._create_placeholder_image(
                                target_w, target_h, f"{name} (Resize Err)"
                            )
                        else:
                    ...
                    self.get_logger().warn(
                        f"Fehler beim Skalieren von Bild '{name}': {e}",
                        throttle_duration_sec=10,
                    )
                    images_to_show.append(
                        self._create_placeholder_image(
                            target_w, target_h, f"{name} (Error)"
                        )
                    )
            else:
                images_to_show.append(
                    self._create_placeholder_image(target_w, target_h, name)
                )
        ...
        if num_images == 0:
            ...
            num_rows_expected = (
                len(self.debug_topics) + self.canvas_cols - 1
            ) // self.canvas_cols
            canvas = self._create_placeholder_image(
                target_w * self.canvas_cols,
                target_h * num_rows_expected,
                "Keine Bilder",
            )
        else:
            ...
            for _ in range(needed_placeholders):
                images_to_show.append(
                    self._create_placeholder_image(target_w, target_h, "")
                )
            ...
            for i in range(num_rows):
                ...
                try:
                    ...
                    self.get_logger().error(f"HStack Fehler Zeile {i}: {e}")
                    rows.append(
                        np.full(
                            (target_h, target_w * self.canvas_cols, 3),
                            self.placeholder_color,
                            dtype=np.uint8,
                        )
                    )
            ...
            if not rows:
                canvas = self._create_placeholder_image(
                    target_w * self.canvas_cols, target_h, "Fehler bei Zeilen"
                )
            else:
                try:
                    canvas = np.vstack(rows)
                except ValueError as e:
                    self.get_logger().error(f"VStack Fehler: {e}")
                    canvas = self._create_placeholder_image(
                        target_w * self.canvas_cols,
                        target_h * len(rows),
                        "Fehler bei Canvas",
            ...
            if scale_factor < 1.0 and canvas.size > 0:
                ...
                if new_h > 0 and new_w > 0:
                    canvas_display = cv2.resize(
                        canvas, (new_w, new_h), interpolation=cv2.INTER_AREA
                    )
                else:
                    canvas_display = (
                        canvas  # Fallback auf Originalgröße bei Skalierungsfehler
                    )
            else:
        ...
        try:
            ...
            self.get_logger().warn(
                "Anzeigefehler (Fenster geschlossen?). Canvas wird deaktiviert.",
                throttle_duration_sec=10,
            )
            self.canvas_visible = False
            self.gui_initialized = False
            ...
            self.get_logger().error(
                f"Canvas Update Fehler: {e}", throttle_duration_sec=10
            )
