...
SPOT_SCAN_ANGLE_MIN_RAD_STRAIGHT = math.radians(SPOT_SCAN_ANGLE_MIN_DEG_STRAIGHT)
SPOT_SCAN_ANGLE_MAX_RAD_STRAIGHT = math.radians(SPOT_SCAN_ANGLE_MAX_DEG_STRAIGHT)
...
class ParkingPhase(Enum):
    IDLE = auto()
    DRIVING_FOR_INITIAL_LASER_SCAN = (
        auto()
    )  # Fährt langsam, bis Laserscan Schild rechts erkennt
    ...
    DRIVING_TO_NEXT_SPOT_ODOM = (
        auto()
    )  # Fährt 33cm zur nächsten Parklücke per Odometrie
...
class ParkingNode(Node):
    def __init__(self):
        ...
        self.current_segment_max_duration = (
            0.0  # Timeout für das aktuelle Odom-Fahrsegment
        )
        ...
        self.maneuver_drive_duration = (
            0.0  # Für zeitbasierte Teile wie Einparken (MOVE_SPOT_DISTANCE)
        )

        self.parking_attempts_count = (
            0  # Zählt Versuche, eine freie Lücke zu finden/anzufahren
        )
        ...
        self.scan_subscriber = self.create_subscription(
            LaserScan, LASERSCAN_TOPIC, self.scan_callback, qos_best_effort
        )
        self.offset_subscriber = self.create_subscription(
            Float64, LANE_OFFSET_TOPIC, self.lane_offset_callback, qos_best_effort
        )
        ...
        self.parking_finished_publisher = self.create_publisher(
    ...
    def robot_state_manager_callback(self, msg: String):
        ...
        if self.current_robot_state_from_manager != new_state_from_manager:
            ...
            if self.current_robot_state_from_manager == STATE_PARKING and (
                self.parking_phase == ParkingPhase.IDLE
                or self.parking_phase == ParkingPhase.MANEUVER_COMPLETE
            ):
                ...
                self.current_robot_state_from_manager != STATE_PARKING
                and self.parking_phase != ParkingPhase.IDLE
            ):
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if (
            self.parking_phase == ParkingPhase.DRIVING_FOR_INITIAL_LASER_SCAN
            and not self.initial_sign_laser_detected
        ):
            obstacle_detected_at_initial_spot = self.check_laser_zone(
                msg,
                INITIAL_SCAN_ANGLE_MIN_RAD,
                INITIAL_SCAN_ANGLE_MAX_RAD,
                INITIAL_SPOT_DETECTION_DISTANCE,
            )
        ...
        # Logik für das Scannen der Parklücke
            ...
            if (
                ...
                if self.parking_attempts_count >= MAX_PARKING_ATTEMPTS:
                    ...
                    self.change_parking_phase(
                        ParkingPhase.DRIVING_TO_NEXT_SPOT_ODOM
                    )  # Odometrie-basierte Fahrt zur nächsten Lücke
    ...
    def change_parking_phase(self, new_phase: ParkingPhase):
        if self.parking_phase != new_phase:
            ...
            )  # Zeitstempel für die neue Phase
            ...
            if new_phase == ParkingPhase.DRIVING_INITIAL_DISTANCE_ODOM:
                ...
                new_phase == ParkingPhase.TURNING_RIGHT_FOR_PARKING
                or new_phase == ParkingPhase.TURNING_LEFT_IN_SPOT_TO_PARK
                or new_phase == ParkingPhase.TURNING_LEFT_IN_SPOT_TO_GET_OUT
                or new_phase == ParkingPhase.TURNING_RIGHT_FOR_LANE_FOLLOWING
            ):
                ...
                if (
                    ...
                    angle_to_turn = TURN_ANGLE_90_DEG
                self.target_yaw_for_turn = self.normalize_angle(
                    self.start_yaw_for_turn + angle_to_turn
                )
                # self.get_logger().info(f"Starte Drehung. Aktueller Yaw: {self.current_yaw:.2f}rad, Ziel-Yaw: {self.target_yaw_for_turn:.2f}rad")
            elif (
                new_phase == ParkingPhase.MOVING_INTO_SPOT
                or new_phase == ParkingPhase.MOVING_OUT_OF_SPOT
            ):
                self.start_pos_x_segment = self.current_pos_x
                self.start_pos_y_segment = self.current_pos_y
                ...
                self.maneuver_drive_duration = (
                    PARKING_DURATION  # Wiederverwendung der Variable für die Wartezeit
                )

            # Stoppe den Roboter bei bestimmten Phasenübergängen
            if new_phase in [
                ParkingPhase.INITIAL_SIGN_STOPPING,
    ...
    def parking_sequence_controller(self):
        ...
        if self.parking_phase == ParkingPhase.IDLE:
            ...
            if elapsed_phase_time >= INITIAL_STOP_DURATION:
                ...
                self.change_parking_phase(
                    ParkingPhase.DRIVING_INITIAL_DISTANCE_ODOM
                )  # Starte 70cm Fahrt
            ...
            if (
                traveled_distance
                >= self.target_distance_segment - ODOM_DISTANCE_TOLERANCE
            ):
            ...
            self.stop_robot()
            if (
                elapsed_phase_time >= STOP_BEFORE_SCAN_DURATION
            ):  # Auch wenn 0, geht direkt weiter
            ...
            if self.turn_to_target(
                self.target_yaw_for_turn, MANEUVER_ANGULAR_SPEED_TURN
            ):
                self.stop_robot()
            ...
            self.stop_robot()
            if (
                elapsed_phase_time >= self.maneuver_drive_duration
            ):  # Hier ist maneuver_drive_duration die PARKING_DURATION
    ...
    def follow_lane_with_recommended_speed(self):
        max_speed_in_parking = self.get_parameter(
            "max_driving_speed_in_parking_mode"
        ).value
        ...
        raw_angular_z = (
            self.current_center_offset * 1.0
        )  # P-Regler für Offset; Faktor ggf. anpassen
        twist_msg.angular.z = float(
            np.clip(
                raw_angular_z, -MAX_ANGULAR_Z_LANE_FOLLOW, MAX_ANGULAR_Z_LANE_FOLLOW
            )
        )
    ...
    def turn_to_target(self, target_yaw: float, angular_speed_abs: float) -> bool:
        ...
        if abs(angle_diff) < GOAL_TOLERANCE_ANGLE_RAD:
            ...
            if (
                angle_diff < 0
            ):  # Ziel ist links (gegen den Uhrzeigersinn für positive Winkel)
                turn_speed_actual = -angular_speed_abs
...
def main(args=None):
    ...
    try:
        ...
        if parking_node:
            parking_node.get_logger().error(
                f"FATAL ERROR in {NODE_NAME}: {e}\n{traceback.format_exc()}"
            )
        else:
            print(
                f"FATAL ERROR in {NODE_NAME} (pre-init or during init): {e}\n{traceback.format_exc()}",
                file=sys.stderr,
            )
    finally:
        if parking_node:
