...
class EdgeDetection:
    ...
    def get_closed_edges(self, **params):
        ...
        blur = cv2.bilateralFilter(
            self.cropped, d=d_value, sigmaColor=sigmaColor, sigmaSpace=sigmaSpace
        )
        ...
        cv2.line(
            edges, (1, self.h - 475), (self.w - 1, self.h - 475), color=255, thickness=1
        )
        cv2.line(
            edges, (1, self.h - 375), (self.w - 1, self.h - 375), color=255, thickness=1
        )
        cv2.line(
            edges, (1, self.h - 250), (self.w - 1, self.h - 250), color=255, thickness=1
        )
        cv2.line(
            edges, (1, self.h - 125), (self.w - 1, self.h - 125), color=255, thickness=1
        )
    ...
    def get_filled_areas(self, **params):
        ...
        contours, _ = cv2.findContours(
            self.closed, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        # --- Alle Flächen füllen ---
        for cnt in contours:
            if cv2.contourArea(cnt) > params.get(
                "min_contour_area"
            ) and cv2.contourArea(cnt) < params.get(
                "max_contour_area"
