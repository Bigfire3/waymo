...
class KeyboardHandlerNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.command_publisher = self.create_publisher(
            String, COMMAND_TOPIC, qos_profile
        )

        self.get_logger().info(f"{NODE_NAME} started.")
        # --- Info-Text angepasst ---
        self.get_logger().info(
            f'Press "{TOGGLE_PAUSE_KEY}" to toggle pause, "{TOGGLE_DEBUG_KEY}" to toggle debug canvas (Ctrl+C to exit).'
        )
        # --- Ende Anpassung ---
        ...
        if sys.stdin.isatty():
            try:
                ...
                self.get_logger().error(
                    f"Is a TTY, but cannot get terminal attributes: {e}"
                )
                rclpy.try_shutdown()
                sys.exit(1)
        else:
    ...
    def listen_keyboard(self):
        ...
        try:
            ...
            while self.listener_thread_active and rclpy.ok():
                ...
                if select.select([sys.stdin], [], [], 0.1) == ([sys.stdin], [], []):
                    ...
                    if key == TOGGLE_PAUSE_KEY:
                        ...
                        self.get_logger().info(
                            f'"{TOGGLE_PAUSE_KEY}" pressed, sending "{cmd_to_send}" command.'
                        )
                    elif key == TOGGLE_DEBUG_KEY:
                        cmd_to_send = "toggle_debug_canvas"
                        self.get_logger().info(
                            f'"{TOGGLE_DEBUG_KEY}" pressed, sending "{cmd_to_send}" command.'
                        )
                    elif key == TOGGLE_LANE_KEY:
                        cmd_to_send = "toggle_lane"
                        self.get_logger().info(
                            f'"{TOGGLE_LANE_KEY}" pressed, sending "{cmd_to_send}" command.'
                        )
                    elif key == TOGGLE_PARKING_KEY:
                        cmd_to_send = "toggle_parking"
                        self.get_logger().info(
                            f'"{TOGGLE_PARKING_KEY}" pressed, sending "{cmd_to_send}" command.'
                        )
                    elif key == TOGGLE_TRAFFIC_LIGHT_KEY:
                        cmd_to_send = "toggle_traffic_light"
                        self.get_logger().info(
                            f'"{TOGGLE_TRAFFIC_LIGHT_KEY}" pressed, sending "{cmd_to_send}" command.'
                        )
                        ...
                        self.get_logger().info(
                            "Ctrl+C detected in thread, stopping listener..."
                        )
                    ...
                    if cmd_to_send:
                        ...
                        try:
                            ...
                            if (
                                ...
                                if rclpy.ok():
                                    self.get_logger().warn(
                                        "Command publisher ist ungültig.",
                            ...
                            if rclpy.ok():
                                self.get_logger().error(
                                    f"Fehler beim Senden des Befehls '{cmd_to_send}': {pub_e}",
                                    throttle_duration_sec=5,
                                )
            ...
            if rclpy.ok():
                self.get_logger().warn(f"Termios error in listener thread: {e}")
        except Exception as e:
            # Andere unerwartete Fehler
            if rclpy.ok():
                self.get_logger().error(
                    f"Exception in keyboard listener thread: {e}",
                    throttle_duration_sec=5,
                )
            ...
            if settings_changed and self.old_term_settings:
                try:
                    termios.tcsetattr(
                        self.stdin_fd, termios.TCSADRAIN, self.old_term_settings
                    )
                except Exception as term_restore_e:
                    # Kann fehlschlagen, wenn Terminal weg ist
                    print(
                        f"WARNUNG: Fehler beim Wiederherstellen der Terminaleinstellungen: {term_restore_e}",
                        file=sys.stderr,
                    )
...
def main(args=None):
    ...
    if is_tty:
        try:
            ...
            print(
                f"FEHLER: Konnte initiale Terminal-Einstellungen nicht lesen: {e}",
                file=sys.stderr,
            )
            is_tty = False  # Behandle als non-tty
    else:
        print(
            "WARNUNG: Läuft nicht in einem TTY. Tastatureingabe wird nicht funktionieren.",
            file=sys.stderr,
        )
    ...
    try:
        if is_tty:
            ...
            print(
                "Beende Keyboard Handler Node, da kein TTY vorhanden ist.",
                file=sys.stderr,
            )
            time.sleep(2)
        ...
        if node:
            pass  # Log im destroy_node
        else:
            print(
                "KeyboardInterrupt erhalten, bevor Node initialisiert wurde.",
                file=sys.stderr,
            )
    except Exception as e:
        ...
        if original_settings:
            try:
                termios.tcsetattr(
                    sys.stdin.fileno(), termios.TCSADRAIN, original_settings
                )
                ...
                print(
                    f"WARNUNG: Fehler beim finalen Wiederherstellen der Terminaleinstellungen: {term_e}",
                    file=sys.stderr,
                )
