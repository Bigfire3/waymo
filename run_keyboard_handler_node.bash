...
SETUP_FILE_SH="$ROS2_WORKSPACE_DIR/install/setup.sh" # Fallback

if [ -f "$SETUP_FILE_BASH" ]; then
#   echo "INFO: Source Workspace Setup-Datei: $SETUP_FILE_BASH"
  source "$SETUP_FILE_BASH"
elif [ -f "$SETUP_FILE_SH" ]; then
#   echo "INFO: Source Workspace Setup-Datei (Fallback): $SETUP_FILE_SH"
  source "$SETUP_FILE_SH"
else
  echo "FEHLER: Keine passende Setup-Datei (setup.bash oder setup.sh) im Verzeichnis $ROS2_WORKSPACE_DIR/install gefunden!"
